...
from openharness.openmontage.lib.pipeline_loader import (
    load_pipeline,
    get_stage_order,
    get_required_tools,
    get_stage_skill,
...
from openharness.openmontage.lib.checkpoint import STAGES
from openharness.openmontage.schemas.artifacts import list_schemas
from openharness.openmontage.styles.playbook_loader import load_playbook, list_playbooks, validate_playbook
from openharness.openmontage.tools.base_tool import ToolTier
from openharness.openmontage.tools.audio.music_gen import MusicGen
from openharness.openmontage.tools.tool_registry import ToolRegistry
from openharness.openmontage.tools.audio.elevenlabs_tts import ElevenLabsTTS
from openharness.openmontage.tools.audio.openai_tts import OpenAITTS
from openharness.openmontage.tools.audio.piper_tts import PiperTTS
...
class TestRemotionScaffold:
    ...
    def test_classroom_experiment_scene_is_wired(self):
        component = self.REMOTION_DIR / "src" / "components" / "ClassroomExperimentScene.tsx"
        index = self.REMOTION_DIR / "src" / "components" / "index.ts"
        explainer = self.REMOTION_DIR / "src" / "Explainer.tsx"
        scene_types = self.REMOTION_DIR / "SCENE_TYPES.md"

        assert component.exists()
        assert "ClassroomExperimentScene" in index.read_text(encoding="utf-8")
        explainer_text = explainer.read_text(encoding="utf-8")
        assert "classroom_experiment" in explainer_text
        assert "ClassroomExperimentScene" in explainer_text
        assert "classroom_experiment" in scene_types.read_text(encoding="utf-8")
...
class TestVideoComposeOperations:
    def test_render_operation_exists(self):
        from openharness.openmontage.tools.video.video_compose import VideoCompose
        tool = VideoCompose()
