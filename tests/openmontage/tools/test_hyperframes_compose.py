...
def test_provider_menu_summary_is_cp1252_safe():
    ...
    import json
    from openharness.openmontage.tools.tool_registry import registry, _scrub_unicode_dashes
...
def test_agent_guide_references_provider_menu_summary():
    ...
    root = Path(__file__).resolve().parents[3] / "src" / "openharness" / "openmontage"
...
def test_style_bridge_fallback_has_all_required_vars():
    from openharness.openmontage.lib.hyperframes_style_bridge import style_bridge
...
def test_composition_validator_hyperframes_asset_root(tmp_path: Path):
    ...
    from openharness.openmontage.tools.analysis.composition_validator import CompositionValidator
