---
name: "hyperframes-registry/references/install-locations"
description: "Install Locations. The `target` field in each item's `registry-item.json` specifies a default install path. The `add` command remaps the prefix based on `hyperframes.json#paths`:"
when_to_use: "When the agent needs guidance for: install locations."
---
